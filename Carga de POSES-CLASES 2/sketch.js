...
function labelToUi(label) {
  if (label === 'A') return 'Correcto';
  if (label === 'B') return 'Incorrecto';
  return label || '—';
}

function setEvalStatus(label) {
  const el = select('#statusEval');
  if (!el) return;
  el.removeClass('is-good');
  el.removeClass('is-bad');
  if (label === 'A') {
    el.html('Aprobado');
    el.addClass('is-good');
  } else if (label === 'B') {
...
function addExample(label) {
  if (!poses || poses.length === 0 || !poses[0] || !poses[0].pose) {
...
function gotResults(err, result) {
  ...
  if (result.confidencesByLabel) {
    ...
    if (result.label) {
      select('#result').html(labelToUi(result.label));
      select('#confidence').html(`${confidences[result.label] * 100} %`);
      setEvalStatus(result.label);
    }
